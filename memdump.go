...
将 Linux 进程的所有可读内存映射依次写入一个原始二进制文件。
...
func run(opts options) (stats dumpStats, returnErr error) {
	...
	for i, m := range mappings {
		if !opts.quiet {
			fmt.Fprintf(os.Stderr, "\r[%d/%d] %016x-%016x %-4s %s", i+1, len(mappings), m.start, m.end, m.permissions, truncatePath(m.pathname, 36))
		}
...
func dumpMapping(mem io.ReaderAt, output io.Writer, m mapping, buffer []byte, pageSize int, strict bool) (uint64, error) {
	...
	for address := m.start; address < m.end; {
		...
		n, readErr := mem.ReadAt(chunk, int64(address))
		if n != want || readErr != nil {
			if strict {
				if readErr == nil {
					readErr = io.ErrUnexpectedEOF
				}
				return unreadable, fmt.Errorf("读取内存 %x-%x: %w", address, address+uint64(want), readErr)
			}
			clear(chunk)
			missing, err := readPageByPage(mem, chunk, address, pageSize)
			if err != nil {
